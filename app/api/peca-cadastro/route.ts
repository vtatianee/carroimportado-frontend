...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.warn("[peca-cadastro] RESEND_API_KEY não configurada — apenas logando.");
      console.log("[peca-cadastro]", body);
    ...
    const htmlBody = `
      <h2>Novo fornecedor de peças — Diretório de Peças</h2>
      <table cellpadding="6" style="border-collapse:collapse;font-family:sans-serif;font-size:14px">
        <tr><td style="color:#64748b;white-space:nowrap"><strong>Fornecedor</strong></td><td>${nome}</td></tr>
      ...
      <p style="color:#94a3b8;font-size:12px">Enviado via carroimportado.com/pecas/cadastro</p>
    `;
    ...
    await resend.emails.send({
      ...
      replyTo: email,
      subject: `Novo fornecedor de peças: ${nome}`,
      html: htmlBody,
    });

    console.log(`[peca-cadastro] Solicitação enviada: ${nome} <${email}>`);
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[peca-cadastro] Erro:", err);
