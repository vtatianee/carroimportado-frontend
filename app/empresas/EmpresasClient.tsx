...
import { EMPRESAS, ESPECIALIDADE_CORES, type Empresa } from "../data/empresas";
    ...
    <article className={`bg-white rounded-2xl border shadow-sm flex flex-col ${empresa.destaque ? "border-blue-200 ring-1 ring-blue-100" : "border-slate-200"}`}>
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <h2 className="font-bold text-slate-900 text-base leading-snug">{empresa.nome}</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              {flag} {empresa.cidade}, {empresa.estado}
            </p>
          </div>
          {empresa.rating !== null && (
            <div className="flex-shrink-0 text-right">
              <StarRating rating={empresa.rating} />
              <p className="text-xs text-slate-500 mt-0.5">
                {empresa.rating.toFixed(1)}
                {empresa.reviews ? ` (${empresa.reviews})` : ""}
              </p>
            </div>
          )}
        </div>
        ...
        {/* Descrição */}
        <p className="text-sm text-slate-600 leading-relaxed flex-1">{empresa.descricao}</p>
...
const ALL_ESPECIALIDADES = Array.from(
  new Set(EMPRESAS.flatMap((e) => e.especialidades))
).sort();
...
export default function EmpresasClient() {
  ...
  const filtered = useMemo(() => {
    return EMPRESAS.filter((e) => {
      ...
      if (espFilter && !e.especialidades.includes(espFilter)) return false;
      if (search) {
        ...
        return (
          ...
          e.especialidades.some((esp) => esp.toLowerCase().includes(q))
        );
      }
      return true;
    });
  ...
  const usCount = filtered.filter((e) => e.pais === "US").length;
  const brCount = filtered.filter((e) => e.pais === "BR").length;

  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      {/* Header */}
      <NavHeader activePage="empresas" />
      ...
      <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 text-white">
        <div className="max-w-5xl mx-auto px-4 py-6 sm:py-10">
          ...
          <p className="text-slate-300 text-sm sm:text-base max-w-2xl">
            Lista curada de exportadoras nos EUA (Flórida) e importadoras no Brasil.
            Pesquise, compare e entre em contato com especialistas no segmento.
          </p>
          <p className="text-xs text-slate-400 mt-3">
            ⚠️ Não temos vínculo comercial com nenhuma das empresas listadas. Faça sua própria pesquisa antes de contratar.
          </p>
        </div>
      </section>
      ...
      <div className="bg-white border-b border-slate-200 sticky top-[57px] z-[9] shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar empresa, cidade, especialidade..."
              value={search}
          ...
          <>
            ...
            {(paisFilter === "todos" || paisFilter === "BR") && brCount > 0 && (
              <section className="mb-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {filtered.filter((e) => e.pais === "BR").map((e) => (
                    <EmpresaCard key={e.id} empresa={e} />
                  ))}
                </div>
              </section>
